...
export function Products() {
  ...
  const [editForm, setEditForm] = useState({ name: "", basePrice: 0, stock: 0, brand: "", description: "", categoryId: "" });
  ...
  const filtered = products.filter(p => {
    ...
    const matchLayout = filterLayout === "all"; // Backend doesn't have layout filtering directly
    const matchStatus = filterStatus === "all" || 
      (filterStatus === "active" && p.stock > 0) ||
      (filterStatus === "inactive" && p.stock === 0);
  ...
  const handleToggleActive = (id: string) => {
    // Just mock for now as we don't have isActive flag in ProductDTO
    toast.success("Tính năng ẩn/hiện sản phẩm chưa được hỗ trợ từ API");
  };

  const totalValue = products.reduce((s, p) => s + p.basePrice * p.stock, 0);
  const activeCount = products.filter(p => p.stock > 0).length; // using stock > 0 as active for now
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button
                          ...
                          className={`rounded-lg ${product.stock > 0 ? 'text-green-600 hover:bg-green-50' : 'text-slate-500 hover:bg-slate-100'}`}
                        >
                          {product.stock > 0 ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>
              ...
              <TabsContent value="info" className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900 mt-1">{viewingProduct.categoryId}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-slate-600">Thương hiệu</Label>
                    <p className="text-slate-900 mt-1">{viewingProduct.brandId}</p>
                  </div>
                  <div>
                      ...
                      <div key={index} className="border border-slate-200 rounded-xl p-4">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <p className="text-slate-900 font-semibold mt-1">{attr.attributeName}</p>
                          </div>
                          <div>
                            <Label className="text-sm font-medium text-slate-600">Giá trị</Label>
                            <p className="text-slate-900 mt-1">{attr.attributeValue}</p>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  value={variantForm.connectionType}
                  onChange={e => setVariantForm({...variantForm, connectionType: e.target.value})}
                  className="rounded-xl"
                  placeholder="VD: Wireless"
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
